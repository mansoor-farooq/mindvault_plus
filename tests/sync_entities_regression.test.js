...
async function runSyncEntitiesRegressionTest() {
  console.log('========================================================================');
  console.log('--- STARTING SYNC ENTITIES REGRESSION TEST ---');
  console.log('========================================================================');

  // 1. Setup temporary test user
  const email = 'test_entity_sync_' + Math.random().toString(36).substring(2) + '@mindvault.io';
  ...
  try {
    ...
    const pushReq = {
      body: {
        ...
        changes: {
          ...
          documents: [{
            id: docSyncId,
            noteId: noteSyncId, // UUID string referencing note
            fileName: 'test.pdf',
          ...
          annotations: [{
            id: annSyncId,
            documentId: docSyncId, // UUID string referencing document
            pageNumber: 3,
    ...
    await syncController.syncData(pushReq, resMock);

    if (resMock.statusCode && resMock.statusCode !== 200) {
      throw new Error(`Sync Push failed with status ${resMock.statusCode}`);
    }
    ...
    await syncController.syncData(pullReq, pullResMock);
...
runSyncEntitiesRegressionTest().catch(err => {
