...
import { ArrowLeft, Rocket, Check, X, ShieldCheck, Database, KeyRound, Save } from 'lucide-react';
...
export default function SaaSManagerPage() {
  ...
  useEffect(() => {
    if (featureAccess) {
      ...
      // By default, if null, all are assumed ON. Let's initialize them ON.
  ...
  return (
    <main className="flex-1 flex flex-col bg-slate-50 min-h-screen">
      <header className="bg-gradient-to-r from-slate-900 to-indigo-950 text-white p-4 sticky top-0 flex items-center justify-between z-10 shadow-xl">
        ...
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-200">
          ...
          <div className="flex flex-col gap-8">
            {TOOL_CATEGORIES.map(cat => {
              if (cat.id === 'dashboard') return null; // Don't allow toggling the main dashboard
                ...
                <div key={cat.id}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {cat.tools.map(tool => {
                      ...
                      const isEnabled = localAccess[tool.featureKey] !== false; // true if missing or true
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h4 className={\ont-bold \\}>{tool.label}</h4>
